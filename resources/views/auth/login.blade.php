...
<body>
    ...
    <div class="login-box">
        <div class="login-logo">
            <a href="#">
                <p class="text-muted" style="height: 20px;"> Login Administator </p>
            </a>
        </div>
        <!-- /.login-logo -->
...
#header-panel{
  font-size: 20px;
    ...
    <script type="text/javascript">
            ...
            function b(b, d) {
                ...
                return d = a.extend({}, a.fn[c].defaults, d), l.prototype.draw = function() {
                    ...
                    for (var a = x.length - 1; a > this.stackPos; a--) {
                        ...
                        f < d.proximity && (q.moveTo(this.position.x + this.parallaxOffsetX, this.position.y + this.parallaxOffsetY), d.curvedLines ? q.quadraticCurveTo(Math.max(b.position.x, b.position.x), Math.min(b.position.y, b.position.y),
                    ...
                    if (d.parallax) {
                        ...
                        this.parallaxTargX = (pointerX - s / 2) / (d.parallaxMultiplier * this.layer), this.parallaxOffsetX += (this.parallaxTargX - this.parallaxOffsetX) / 10, this.parallaxTargY = (pointerY - t / 2) / (d.parallaxMultiplier *
                            this.layer), this.parallaxOffsetY += (this.parallaxTargY - this.parallaxOffsetY) / 10
                    }
                    switch (d.directionX) {
                        case "left":
         ...
         */
        function() {
            for (var a = 0, b = ["ms", "moz", "webkit", "o"], c = 0; c < b.length && !window.requestAnimationFrame; ++c) window.requestAnimationFrame = window[b[c] + "RequestAnimationFrame"], window.cancelAnimationFrame = window[b[c] +
                "CancelAnimationFrame"] || window[b[c] + "CancelRequestAnimationFrame"];
